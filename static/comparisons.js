...
function initialize() {
    ...
    //completed search, aka search button pressed, left + right
    ...
    //completed search, aka enter key pressed, left + right
...
function loadHeading(video_game_1, video_game_2){
    //loads the heading. accounts for if each video does not exist.

    if (video_game_1 != null && video_game_1 != ""){
        //testing if video game 1 exists as a string and as a game in the database.
        //show actual game if exists, show what was searched if not.
    ...
    if (video_game_2 != null && video_game_2 != ""){
        //testing if video game 2 exists as a string and as a game in the database.
        //show actual game if exists, show what was searched if not.
...
function loadGameBasic(url, div){
    //loads the game and prepares it for further use in other functions
    let game_data = [];
    fetch(url, {method: 'get'})
    ...
    .then(function(result){
        if (Object.keys(result).length >= 2){
            ...
            //saving information to game_data as well so we can use it after in other functions
...
function displayComparisons(game_data, div){
    //we have the infromation, now lets just display it.
    let game_info = "";
...
function selectPlatforms(game_data, div){
    //instead of showing all the information, letting the user pick the platform for each side will
    //make it easier for them to compare. So, let's load those selections.
...
function loadGamePlatform(url, div){
    //once a platform has been selected, let's load the infromation.
    fetch(url, {method: 'get'})
    ...
    .then(function(game_data){
        ...
        if (Object.keys(game_data).length >= 2){
            ...
            for (let i = 1; i < game_data.length; i++){
                //we don't need to save the information this time since this is the last thing,
                //so just display it.
