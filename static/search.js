...
// SEARCH SPECIFIC
function loadHeading(search, results){
    let heading = document.getElementById('heading');
    if (heading && results > 1){
        heading.innerHTML = results + ' results for "' + search + '":';
    } else if (heading && results == 1){
        heading.innerHTML = results + ' result for "' + search + '":';
    } else {
        heading.innerHTML = results + ' results for "' + search + '":';
    }
}
...
function loadResults() {
    ...
    if (search != "" && search != null){
      ...
      .then(function(videoGames) { //display the results
          ...
          loadHeading(search, videoGames.length);
